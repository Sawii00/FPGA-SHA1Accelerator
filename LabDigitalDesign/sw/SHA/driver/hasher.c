...
#define DRIVER_WITH_INTERRUPT 0
...
static int hasher_init(void)
{
    ...
    result = request_irq(hasher_mem.irq, (irq_handler_t)hasherIRQHandler, IRQF_TRIGGER_RISING, DRIVER_NAME, &hasher_mem); if(result) {
...
static irq_handler_t hasherIRQHandler(unsigned int irq, void *dev_id, struct pt_regs *regs)
{
    if (irq != HASHER_IRQ)return IRQ_NONE;
